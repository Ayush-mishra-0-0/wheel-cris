...
Why we retrain instead of loading a saved model:
  ...
  (experiment_0020 / experiment_0015) on the train split and predicts on all
  three splits, so the whole thing is reproducible from the released
  model_dataset alone.
...
# ---------------------------------------------------------------------------
# Paths — adjust PROJECT_ROOT if this script isn't run from the repo root.
# ---------------------------------------------------------------------------
...
def plot_single_wheelset(dataset: pd.DataFrame, equipment_ids: dict[str, int] | None = None) -> Path:
    ...
    ax.set_title("Wheel diameter over time — holdout split actual vs predicted-next")
    ax.xaxis.set_major_formatter(mdates.DateFormatter("%Y-%m"))
    ax.legend(loc="best", fontsize=8, ncol=2)
    fig.autofmt_xdate()
    fig.tight_layout()

    out_path = OUTPUT_DIR / "diameter_timeseries_wheelset_per_split.png"
...
def plot_fleet_monthly_aggregate(dataset: pd.DataFrame) -> Path:
    ...
    ax.set_title("Fleet-wide mean diameter over time — actual (solid) vs predicted-next (dashed)")
    ax.xaxis.set_major_formatter(mdates.DateFormatter("%Y-%m"))
    ax.legend(loc="best", fontsize=8, ncol=2)
    fig.autofmt_xdate()
    fig.tight_layout()

    out_path = OUTPUT_DIR / "diameter_timeseries_fleet_monthly.png"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(out_path, dpi=150)
    plt.close(fig)
    return out_path
